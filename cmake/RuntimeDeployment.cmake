...
function(threeviewer_runtime_post_build target)
  ...
  if(_osg_plugins_dir STREQUAL "")
    file(GLOB _osg_plugins_dirs_release "${_runtime_root}/plugins/osgPlugins-*")
    file(GLOB _osg_plugins_dirs_debug "${_runtime_root}/debug/plugins/osgPlugins-*")
    if(_osg_plugins_dirs_release)
      list(GET _osg_plugins_dirs_release 0 _osg_plugins_dir_release)
    endif()
    if(_osg_plugins_dirs_debug)
      list(GET _osg_plugins_dirs_debug 0 _osg_plugins_dir_debug)
    endif()
    if(_osg_plugins_dir_release AND _osg_plugins_dir_debug)
      set(_osg_plugins_dir "$<IF:$<CONFIG:Debug>,${_osg_plugins_dir_debug},${_osg_plugins_dir_release}>")
    elseif(_osg_plugins_dir_debug)
      set(_osg_plugins_dir "${_osg_plugins_dir_debug}")
    elseif(_osg_plugins_dir_release)
      set(_osg_plugins_dir "${_osg_plugins_dir_release}")
    endif()
  endif()

  if(_osg_plugins_dir)
    get_filename_component(_osg_plugins_name "${_osg_plugins_dir_release}" NAME)
    if(NOT _osg_plugins_name AND _osg_plugins_dir_debug)
      get_filename_component(_osg_plugins_name "${_osg_plugins_dir_debug}" NAME)
    endif()
    add_custom_command(TARGET ${target} POST_BUILD
      COMMAND ${CMAKE_COMMAND} -E copy_directory
        "${_osg_plugins_dir}"
